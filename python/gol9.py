...
Uses only immutable structures.
...
def updateCellCount(cells, cell, isAlive):
    if not cells:
        ...
        head = cells[0]
        (headCell, headIsAlive, headCount) = head
        if cell == headCell:
            newHead = (headCell, headIsAlive, headCount+1)
            return (newHead,) + cells[1:]
        else:
            return (head,) + updateCellCount(cells[1:], cell, isAlive)
...
def nextBoard(livingCells=[]):
    return [cell for (cell, alive, count) in allCellsWithCounts(livingCells) if shouldLive(count, alive)]
